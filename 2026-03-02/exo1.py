...
# le signal SIGUSR1 au père puis se termine.
...
# modifier le fils de telle sorte qu'il envoie à la suite n signaux SIGUSR1 au père avant
# de se terminer.
# 4. Modifier le père pour qu'il compte le nombre de signaux SIGUSR1 reçus ; supprimer
# tout affichage dans le handler de signal du père, et afficher le nombre final de signaux
...
if fork_result == 0:
    ...
    for i in range(n):
        ...
        time.sleep(0.1) # pour espacer les envois de signaux et laisser le temps au pere de réagir
    sys.exit(0)
# père
os.wait()
